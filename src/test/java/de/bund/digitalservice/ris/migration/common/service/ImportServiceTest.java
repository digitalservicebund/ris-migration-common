...
class ImportServiceTest {
	...
	@Mock
	private ObjectProvider<S3MigrationService> s3ServiceProvider;
	...
	void importData_localMode_skipsS3(@TempDir Path tempDir) throws Exception {
		...
		service.importData(new ExecutionContext(), "daily", tempDir.toString(), LocalDate.now());

		verify(s3MigrationService, never()).downloadFolder(org.mockito.ArgumentMatchers.any(),
	...
	void importData_dailyCloudMode_downloadsFromS3() throws Exception {
		...
		when(s3MigrationService.resolveDailySourcePath(LocalDate.of(2025, 1, 15))).thenReturn("daily/2025/01/14/");

		var context = new ExecutionContext();
		service.importData(context, "daily", "/tmp/input", LocalDate.of(2025, 1, 15));
	...
	void importData_monthlyCloudMode_downloadsFromS3() throws Exception {
		when(s3ServiceProvider.getIfAvailable()).thenReturn(s3MigrationService);
		var monthlySource = new S3MigrationService.MonthlyImportSource("monthly/2025/01/", LocalDate.of(2024, 12, 26));
		when(s3MigrationService.resolveMonthlyPrefix(LocalDate.of(2025, 1, 15))).thenReturn(monthlySource);

		var context = new ExecutionContext();
		service.importData(context, "monthly", "/tmp/input", LocalDate.of(2025, 1, 15));
	...
	void importData_localMode_missingDirectory_doesNotThrow() throws Exception {
		...
		service.importData(new ExecutionContext(), "daily", "/nonexistent/path", LocalDate.now());
	}
}
