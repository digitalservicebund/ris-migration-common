...
class PublishTaskletTest {
  ...
  void execute_dailyMigration_uploadsAndSetsDailyExitStatus() throws Exception {
    ...
    StepExecution stepExecution = MetaDataInstanceFactory.createStepExecution(new JobParametersBuilder()
            .addString("migrationType", MigrationType.DAILY.name()).toJobParameters());
    ...
    assertThat(stepContribution.getExitStatus()).isEqualTo(new ExitStatus(PublishTasklet.DAILY_MIGRATION_COMPLETED));
  ...
  void execute_monthlyMigration_uploadsAndSetsMonthlyExitStatus() throws Exception {
    ...
    var tasklet = new PublishTasklet(s3MigrationService, "/output");
    StepExecution stepExecution = MetaDataInstanceFactory.createStepExecution(new JobParametersBuilder()
            .addString("migrationType", MigrationType.MONTHLY.name()).toJobParameters());
    StepContribution stepContribution = new StepContribution(stepExecution);
    ChunkContext chunkContext = new ChunkContext(new StepContext(stepExecution));
    ...
    assertThat(stepContribution.getExitStatus()).isEqualTo(new ExitStatus(PublishTasklet.MONTHLY_MIGRATION_COMPLETED));
  }
